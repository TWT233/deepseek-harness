...
The shipped `cli` bundle sets sandbox mode `danger-full-access`, approval policy `never`, and disables the permission-selector service. The approval service remains mounted for tool execution, but no approval answerer or approval UI is registered. The startup block states the effective policy values; with the shipped defaults it warns that commands and tools can modify any path available to the process. Later profile, home, or command-line patches can replace the policy rows, and a fresh Session records those effective service values. The `ctx.userQuestions` provider remains interactive for model-requested structured questions.
